  ...
  <div class="min-h-screen bg-slate-950 text-slate-100 font-sans antialiased p-8 flex flex-col items-center">
    <div class="max-w-4xl w-full">
      ...
      >
        ...
        <p class="text-slate-400 mt-4 text-sm font-semibold">Carregando detalhes do filme...</p>
      </div>
      ...
      >
        <h2 class="text-lg font-bold text-red-400 mb-2">Erro ao carregar o filme</h2>
      ...
      >
        ...
        <div class="p-6 md:p-8 border-b border-white/5 bg-slate-950/40">
          <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
            <div>
              <span class="px-2.5 py-1 bg-purple-500/10 border border-purple-500/25 rounded-sm text-[10px] font-bold text-purple-400 uppercase tracking-wider">
                ID: {{ movie.id }}
              </span>
              <h1 class="text-3xl md:text-4xl font-extrabold text-white mt-2 leading-tight">
                {{ movie.title }}
              </h1>
              <p v-if="movie.tagline" class="text-purple-400 italic text-sm mt-1">
                "{{ movie.tagline }}"
              </p>
            </div>
            
            <div class="flex items-center gap-2 self-start md:self-auto">
              <div class="px-4 py-2 bg-amber-500/10 border border-amber-500/20 rounded-sm text-center">
                <span class="block text-[10px] uppercase font-bold text-amber-400 tracking-wider">Nota</span>
                <span class="text-2xl font-black text-amber-400">
                  ★ {{ movie.vote_average ? parseFloat(movie.vote_average).toFixed(1) : 'N/A' }}
                </span>
              </div>
              <div class="px-4 py-2 bg-slate-800 border border-white/5 rounded-sm text-center">
                <span class="block text-[10px] uppercase font-bold text-slate-400 tracking-wider">Votos</span>
        ...
        <div class="p-6 md:p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div class="md:col-span-2 space-y-6">
            ...
            <div v-if="movie.keywords && movie.keywords.length > 0">
              <h2 class="text-sm font-bold text-slate-400 uppercase tracking-wider mb-3">Palavras-chave</h2>
          ...
          <div class="bg-slate-950/40 border border-white/5 rounded-sm p-5 space-y-4 text-sm text-slate-400">
            <h2 class="text-white font-bold text-sm border-b border-white/5 pb-2 uppercase tracking-wider">
              Ficha Técnica
            </h2>

            <div class="space-y-3">
              <div class="flex justify-between items-center py-1 border-b border-white/5">
                <span class="font-semibold text-slate-300">Título Original:</span>
                <span class="text-right text-slate-200 truncate max-w-[180px]">{{ movie.original_title || 'N/A' }}</span>
              </div>
              <div class="flex justify-between items-center py-1 border-b border-white/5">
                <span class="font-semibold text-slate-300">Idioma Original:</span>
                <span class="text-slate-200 uppercase">{{ movie.original_language || 'N/A' }}</span>
              </div>
              <div class="flex justify-between items-center py-1 border-b border-white/5">
                <span class="font-semibold text-slate-300">Status:</span>
                <span class="px-2 py-0.5 bg-green-500/10 border border-green-500/20 text-green-400 text-xs rounded-sm">
                  {{ movie.status || 'N/A' }}
                </span>
              </div>
              <div class="flex justify-between items-center py-1 border-b border-white/5">
              ...
              <div class="flex justify-between items-center py-1 border-b border-white/5">
                ...
                <span class="text-slate-200">
                  {{ movie.runtime ? `${Math.floor(movie.runtime / 60)}h ${Math.round(movie.runtime % 60)}m` : 'N/A' }}
                </span>
              </div>
              <div class="flex justify-between items-center py-1 border-b border-white/5">
            ...
            <!-- Site Oficial -->
            <div v-if="movie.homepage" class="pt-4 border-t border-white/5 text-center">
              <a
                :href="movie.homepage"
                target="_blank"
        ...
        <!-- Produtoras e Países -->
        <div class="px-6 md:p-8 pb-8 pt-4 border-t border-white/5 bg-slate-950/20 text-xs text-slate-400 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div v-if="movie.production_companies && movie.production_companies.length > 0">
            <span class="font-bold text-slate-300 uppercase tracking-wider block mb-2">Produtoras</span>
          ...
          <div v-if="movie.production_countries && movie.production_countries.length > 0">
            <span class="font-bold text-slate-300 uppercase tracking-wider block mb-2">Países Produtores</span>
            <div class="flex flex-wrap gap-2">
              <span
...
const movie = computed(() => {
  return mdbMovieSelect.response?.[0] || null;
});
