  ...
  const addWidget = (widget: WidgetDefinition) => {
    setActiveWidgets((prev) => [
      ...prev,
      {
        ...
        widgetId: widget.id,
        props: { ...widget.props },
        isEditing: true, // Começa em modo de edição para o usuário configurar
      },
    ]);
        ...
        <div className="relative">
          ...
          {isPickerOpen && (
            <div className="absolute right-0 mt-2 w-56 bg-zinc-900 border border-zinc-800 rounded-xl shadow-xl overflow-hidden z-10">
              <div className="p-2">
                <h3 className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-2 px-2">Widgets Disponíveis</h3>
                {widgets.map((widget) => (
                  <button
                    key={widget.id}
                  ...
                  >
                    <span className="text-zinc-200 text-sm font-medium">{widget.name}</span>
                    <span className="text-zinc-500 text-xs">{widget.description}</span>
                  </button>
                ))}
              </div>
            </div>
      ...
      {activeWidgets.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {activeWidgets.map((activeWidget) => {
            const definition = widgets.find((w) => w.id === activeWidget.widgetId);
            ...
            return (
              <div key={activeWidget.instanceId} className="relative group">
                ...
                <WidgetComponent 
                  ...
                  onSave={(newProps) => updateWidgetProps(activeWidget.instanceId, newProps)}
